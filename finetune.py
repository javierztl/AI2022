...
if __name__ == '__main__':
	...
	def model():
		base_model = tf.keras.models.load_model('./facemask_personid_classifier_best.h5', custom_objects={'CircleLoss': CircleLoss(gamma=64, margin=0.25, batch_size=64)});
		person_identifier = tf.keras.layers.Dense(3, activation="softmax", name='ID_Classifier', kernel_regularizer=tf.keras.regularizers.l2())(base_model.get_layer('flatten_2').output);
		
		
		

		model = tf.keras.Model(inputs = base_model.input, outputs = [base_model.get_layer('dense_1').output, person_identifier]);
	...
	(image_train, image_val, label_mask_train, label_mask_val, label_id_train, label_id_val) = train_test_split(data, label_mask, label_id, test_size = 0.15);
	...
	label_id_train_onehot = tf.keras.utils.to_categorical(label_id_train, num_classes = 3);
	label_id_val_onehot = tf.keras.utils.to_categorical(label_id_val, num_classes = 3);
	...
	model = model();
	model.summary();
	checkpointer = tf.keras.callbacks.ModelCheckpoint(filepath='facemask_personid_classifier_fine_best.h5', save_best_only=True);
	history = model.fit(
	...
	model.save('facemask_personid_classifier_fine.h5');
